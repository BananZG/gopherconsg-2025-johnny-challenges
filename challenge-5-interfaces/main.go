...
func (b base) baseCount(condition func(rune) bool, input string) int {
	ret := 0
	for _, c := range input {
		if condition(c) {
			ret++
...
func (l letterCounter) count(input string) int {
	return l.baseCount(unicode.IsLetter, input)
}
...
func (n numberCounter) count(input string) int {
	return n.baseCount(unicode.IsNumber, input)
}
...
func (s symbolCounter) count(input string) int {
	return s.baseCount(func(r rune) bool {
